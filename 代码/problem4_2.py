...
六、交付表的购电量里有 **12.37%** 既未供负载、也未进电池（必须声明）
    实测（交付读法 `G` 无上界，口径同 diag_p4_audit.py）：年购电 21,734,671 kWh，而净需求
    (L−P)Δt 仅 17,939,189 kWh；扣掉充 6,396,664 / 放 5,183,584 / 紧急 105,864 之后仍差
    −2,688,266 kWh，即 12.37% 的购电量无处安放（买了但用不上）。
    ⚠ **丢弃的那部分不是上界造成的**：交付读法下 max g = 1,720.89 kWh/槽 = 10,325.3 kW，
    上界已放开，故**不存在"贴上限"**；有 15.21% 的槽（7,313/48,096）会**越过** 5,000 kW
    这条自加假设线 —— 这正是交付不采用 `G≤5000` 的原因（§五）。
    （旧版本此处写「g 有 36.61% 的槽贴在 G_MAX=5,000 kW 上 ⇒ 上界是紧的」，
     那是 `G≤5000` 时代的数，交付读法一变就失效。）
    这是 problem2.py 继承下来的口径性质（result2.xlsx 同源），不是问题 4 引入的。
