# -*- coding: utf-8 -*-
"""问题 3 —— 口径 A 下的**定稿求解器**（追索变量 + 购电无上界）

★★ 本文件是问题 3 的正式交付脚本，产出 `结果/result3.xlsx` = **13,630,568 元**。
   一键复现：`python 代码/run_problem3.py`。

★ 本文件 = CUMCM-C/代码/problem3_v3.py 的**唯一主改动**：把问题 3 计划层的充放电
  (c,d) 改成问题 2 的**追索变量**（逐情景 ω 的二阶段变量），其余全部保留 ——
  预测 F̂、分位缓冲 h、6/12/18 调整层、口径 A 结算、低需求日分组 span7。
  环境变量 P3_REC 切换：=1 走追索计划 plan_day_recourse + 因果执行 exec_causal_day
  （缺省）；=0 退回原问题 3 的确定性 plan_day（= 代码/problem3_v3.py，作 A/B 对照）。

  ⚠ **交付配置还要求 `P3_GMAX=20000`（购电无上界，字面读法）。**
    缺省 `P3_GMAX=P_MAX`(=5000) 是旧的保守读法，**只会得 14,376,005，不是交付值**。
    两种 `G_MAX` 读法不可混排：不能拿本文件的无上界交付值去比对问题 4 的有上界交付值
    （见 文档/taskrun.md §5.7）。启动时会打印一行「本配置是否为交付配置」的自检。

  ⚠⚠ **换计划层之后 τ 必须重调。** 旧 τ=(0.80, 0.55) 属确定性计划时代，原样搬过来
    偏贵 185,974 元。定稿 τ=(0.55, 0.42, 0.42, 0.42)，扫描证据见参数区注释与
    文档/问题3_求解归档.md §5.2。
...
§1  v2 的病根（为什么要另起）
════════════════════════════════════════════════════════════════════════════
交付物 result3.xlsx（14,722,577.56 元）已定位为 v2 的 `PLAN=lp EXEC_NAIVE=1`（复现
...
决定。** 实测 RMSE 矩阵（净需求 kW，2/1–12/31，各档只在其覆盖时段评）：
  ...
  ⇒ 0:00 档与"同星期几持续性"打平（无增量）；6:00 与 12:00 档把误差砍半；18:00 档相对
    当日 0:00 档亦无增量。故第二问应按**档位分开回答**。v3 用 P3_BANDS 分别报：
    BANDS=0（只用 0:00 档）／1（+6:00）／2（+12:00）／3（+18:00）四条线的费用差。

════════════════════════════════════════════════════════════════════════════
用法
════════════════════════════════════════════════════════════════════════════
    python 代码/problem3_v3.py                  # 全年，扫 λ
    P3_LAM=0.6 python 代码/problem3_v3.py       # 单一 λ
    P3_ADJ=0 python 代码/problem3_v3.py         # 关掉调整通道
    P3_BANDS=0/1/2/3                            # 允许的最高档位
    P3_DAYS=60 python 代码/problem3_v3.py       # 只跑前 60 天（计时）
    P3_SENT=0 python 代码/problem3_v3.py        # 跳过哨兵（哨兵较慢）
"""
...
# g = 净负荷 + 5000 kW > 5000 kW。按字面读法放开上界：P3_GMAX=inf。
G_MAX = float(os.environ.get("P3_GMAX", P_MAX))
SOC_MIN = 1200.0
SOC_MAX = 10800.0
SOC0 = 6000.0
N = 144
...
TAU = float(os.environ.get("P3_TAU", 0.55))
# τ' 是**调整层**的分位（6/12/18 档把后续时窗的目标净需求抬到哪一分位）。
#
# ⚠⚠ 2026-09-12：τ 在**换计划层之后必须重调**，旧值 (0.80, 0.55) 是确定性计划时代的定稿，
#    搬到追索计划下已偏贵 185,974 元（13,822,501 vs 13,630,568）。实测最优面板（span7、
#    REC=1、GMAX=20000）：
#      τ'   : 0.10→15,521,802  0.20→14,323,429  0.30→13,853,996  0.35→13,751,122
#             0.40→13,636,527  0.42→**13,630,568**  0.44→13,631,482  0.46→13,640,480
#             0.48→13,652,619                                    （以上固定 τ₀=0.55）
#             0.55→13,822,501  0.60→13,914,302          （τ₀=0.80，即**旧配置**那一列）
#      τ₀   : 0.40→13,653,915  0.45→13,636,546  0.50→13,629,951  0.52→13,629,186
#             0.55→**13,630,568**  0.58→13,634,055  0.60→13,637,105  0.65→13,649,560
#             0.70→13,663,875  0.80→13,701,315  0.90→13,756,354  1.00→14,192,190
#                                                            （以上固定 τ'=0.42）
#    两维都是**内点最优**（τ'=0.10 处炸到 15,521,802，紧急 471,422 kWh）。平台
#    0.50–0.55 × 0.42–0.44 极差仅 2,296 元（0.017%）；网格最小值在 (0.52, 0.42)=13,629,186，
#    比定稿仅低 1,382 元（0.010%），为不overfit 网格，block-0 保持 0.55。
#
#    τ' 从 0.55 降到 0.42 是**有理论方向的**：纯理论临界比 0.5p/(0.5p+p)=1/3，旧值 0.55
#    之所以被抬高，是因为确定性计划层欠对冲、逼着调整层过度补偿；追索计划层自己就按情景
#    对冲了可靠性，调整层的边际职责回到接近纯理论的位置。
TAUP = float(os.environ.get("P3_TAUP", 0.42))
# 分块临界分位 [0:00块, 6:00块, 12:00块, 18:00块]。缺省 (0.55, 0.42, 0.42, 0.42) 是定稿值。
# 0:00–6:00 那一块**不可调整**（6:00 档发布前已执行完），缺口只能用 5p 紧急购电补，
# §0 的静态推导给临界比 = 4p/(4p+p) = 0.80。**但实测最优是 0.55，不是 0.80**：
# 那一步推导假定 ĝ 是第 0 块的唯一对冲手段，而追索计划里**电池的充放电 c/d 本身逐情景
# 对冲**，把两侧代价削平，有效临界分位因此下移。**所以 τ 只能扫、不能推。**
# 用全天单一 τ 会把这个结构差别抹平，正是单一 τ 下总费在 0.65–0.70 压成平台的原因。
TAU_B = [float(x) for x in os.environ.get("P3_TAUB", "").split(",")] if os.environ.get("P3_TAUB") \
    else [0.55, 0.42, 0.42, 0.42]
assert len(TAU_B) == 4, "P3_TAUB 需给 4 个数"
# λ 只在 EXEC="mpc"（自解执行）下起作用；缺省 EXEC="plan"（跟随计划充放电）时
# 执行轨迹由 plan_day 的 LP 唯一确定，λ 完全不进模型 ⇒ 扫多个 λ 只会得到同一串数。
LAMS = [float(x) for x in os.environ.get("P3_LAMS", "1.0").split(",")]
LAM_SET = os.environ.get("P3_LAM")
ADJ = bool(int(os.environ.get("P3_ADJ", 1)))
LEVEL = bool(int(os.environ.get("P3_LEVEL", 1)))   # 调整层是否做"已发生比值"水平校正
BANDS = int(os.environ.get("P3_BANDS", 3))
NDAYS_RUN = int(os.environ.get("P3_DAYS", NDAYS))
MINR = int(os.environ.get("P3_MINR", 14))
MPC_RES = int(os.environ.get("P3_RES", 1))
H_MAX = int(os.environ.get("P3_H", 144))
DO_SENT = bool(int(os.environ.get("P3_SENT", 1)))
DO_WRITE = bool(int(os.environ.get("P3_WRITE", 1)))   # 跑完把最优配置写 结果/result3.xlsx
PLAN = os.environ.get("P3_PLAN", "lp")      # "lp" = 日前套利 LP ｜ "quant" = 追负荷分位规则
EXEC = os.environ.get("P3_EXEC", "plan")    # "plan" = 跟随计划充放电 ｜ "mpc" = 只收 ĝ 自己重解
# ★ 本文件的唯一主改动：把日前计划的充放电 (c,d) 改成问题 2 的追索变量（逐情景 ω），
#   其余（预测 F̂、分位缓冲 h、6/12/18 调整层、口径 A 结算、低需求日分组 span7）全部保留。
#   P3_REC=0 可退回原问题 3 的确定性计划（= problem3_v3.py），用于 A/B 对照。
#
#   ⚠ 问题 3 的**正式交付**配置 = P3_REC=1 且 P3_GMAX=20000（追索 + 购电无上界）
#     且 τ=(0.55, 0.42, 0.42, 0.42)，产出 结果/result3.xlsx = 13,630,568 元。
#     一键复现见 代码/run_problem3.py（它把四个旋钮全部钉死）。
#     缺省 P3_GMAX=P_MAX(=5000) 是**旧的保守读法**，产出的是 14,376,005，
#     已被取代，勿再当作交付值引用。
REC = bool(int(os.environ.get("P3_REC", 1)))
...
def exec_causal_day(d, g_day, soc):
    ...
    追索版里计划层只输出 ĝ（无单一实值的 c/d），执行必须因果落地：每槽只用当前实际
    L/P、当日 ĝ 与当前 SOC —— 先放覆盖缺口、富余再充、仍缺才紧急。它与追索 LP 的
    "充电只吃富余 + 负荷必须供上"两条约束逐槽对应（c ≤ ĝ−N+d、e ≥ N−ĝ−d），
...
def _scenarios(d, Nt):
    ...
    与问题 2 追索模型同一思路 —— 情景取"历史同组日"，但问题 3 要保留预报，故把历史
    同伴的净需求**平移**到当日预报水平：Ns[ω] = Nt + (actual_net[peer_ω] − 同伴均值)。
    即保留当日预测 F̂（预测不丢），只用历史同伴提供"逐槽误差形状"。
...
def run(lam, ndays=None, verbose=False):
    ...
    for d in range(nd):
        ...
        if len(H0) > 0:
            for e in range(4):
                a, b = BLOCKS[e]
                h[a:b] = np.quantile(H0[:, a:b], TAU_B[e], axis=0)
        Nt = F_hat[d, 0, :] + h
        ...
        if ADJ and len(src) >= MINR:
            for e in (1, 2, 3):
                ...
                Nt_adj[a:b] = np.maximum(Nt_adj[a:b], Ncor + np.quantile(Rcor, TAUP, axis=0))
        ...
        def _solve(target):
            ...
            REC=0 时退回原问题 3 的确定性 plan_day（= CUMCM-C 原版），用于 A/B 对照。
            """
            if PLAN == "lp":
                if REC:
                    pl = plan_day_recourse(soc, _scenarios(d, target), price_day, SOC0)
                    if pl is not None:
                        return np.clip(pl, 0.0, G_MAX), None, None
                else:
                    pl = plan_day(soc, target, price_day, SOC0)
                    if pl is not None:
...
# ════════════════════════════════════════════════════════════════════════
# 落盘：结果/result3.xlsx（沿用 附件5/result3.xlsx 模板）
# ════════════════════════════════════════════════════════════════════════
...
def write_result3(r, out_path=None):
    """把 run() 的结果按 附件5/result3.xlsx 模板写盘。账本是**口径 A**。
    ...
    def day_fee(di):
        ...
        return float((price_day * gh).sum()
                     + (0.5 * price_day * np.maximum(gh - gf, 0)).sum()
                     + (1.5 * price_day * np.maximum(gf - gh, 0)).sum()
                     + (5.0 * price_day * ed).sum())

    out_path = out_path or os.environ.get("P3_OUT") or os.path.join(BASE, "结果", "result3.xlsx")
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    wb = openpyxl.load_workbook(os.path.join(BASE, "附件", "附件5", "result3.xlsx"))
...
if __name__ == "__main__":
    print("=" * 118)
    print("问题 3 · 口径 A · 独立重解（problem3_v3 · 追索变量版）")
    print(f"  计划层充放电：{'追索变量（问题2两阶段）' if REC else '确定性 plan_day（原版）'}"
