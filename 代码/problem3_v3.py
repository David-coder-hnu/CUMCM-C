# -*- coding: utf-8 -*-
"""问题 3 —— 口径 A 下的独立重解（problem3_v3）· ⚠ 已非定稿

⚠⚠ **本文件产出的是旧值 14,285,731 元（确定性计划 + 购电 G≤5000），已不是问题 3 的
   交付值。** 2026-09-12 问题 3 改用「**追索变量 + 购电无上界 + 重调 τ**」，
   交付值 **13,630,568 元**（见 `文档/问题3_求解归档.md`）。

   ⟹ 要复现交付值请跑 `python 代码/run_problem3.py`（脚本 `代码/problem3_recourse.py`）。
   ⟹ `problem3_recourse.py` 是本文件的**严格超集**：`P3_REC=0` 精确复现本文件的 14,285,731，
      故本文件保留仅为 **A/B 对照**（`文档/taskrun.md` §5.7 四象限的左列）与历史留痕，
      **不要再用它产出交付表**。
   ⟹ ⚠ 与追索版比较时**必须各自调平 τ**：本文件的 τ=(0.80, 0.55) 是确定性计划层的最优，
      搬到追索版偏贵 185,974 元。**跨计划层直接比总费会用错误的 τ 诋毁对方。**
...
def write_result3(r, out_path=None):
    ...
    out_path = out_path or os.path.join(BASE, "结果", "result3.xlsx")
...
if __name__ == "__main__":
    print("=" * 118)
    print("问题 3 · 口径 A · 独立重解（problem3_v3）")
