...
#   其余（预测 F̂、分位缓冲 h、6/12/18 调整层、口径 A 结算、G≤5000、分组 span7）全部保留。
#   P3_REC=0 可退回原问题 3 的确定性计划（= CUMCM-C 原版），用于 A/B 对照。
