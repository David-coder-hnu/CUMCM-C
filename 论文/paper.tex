...
% 超链接配色：cls 里设了 colorlinks + allcolors=black（可点但不可见），
% 这里把三类链接改成可见色——图表/公式/节引用深蓝，文献引用深红。
\hypersetup{%
  colorlinks=true,
  linkcolor=blue!55!black,
  citecolor=red!55!black,
  urlcolor=blue!55!black,
  filecolor=blue!55!black,
  anchorcolor=blue!55!black}
...
推出\textbf{调整只允许上调、违约费恒为零}（实测 0 元）。全年总购电费 \textbf{13\,641\,420 元}，
高出同口径完美预见下界 \textbf{$11.55\%$}；消融显示日内调整通道值 104.87 万元、
水平校正 8.44 万元、低需求日分组 60.35 万元。

\textbf{问题四}换用附件 4 的逐日逐槽实际电价。方差分解显示日内形状占 \textbf{$84.6\%$}、
日水平因子仅占 \textbf{$9.2\%$}，而四项收费同为电价的倍数、日水平又只是全天均匀平移，
故在\emph{同一分位口径}下电价的整体水平\textbf{只改变账单、不改变决策}——实测换价后
各决策量\textbf{逐元素不变}；但价格结构与缺口的相关性会移动\emph{最优分位本身}，
故问题四的交付分位经五维重扫后与问题三不同，两者的交付文件据此逐格相异。

灵敏度分析表明：模型对分位参数与电价水平都不敏感（分位平台总费极差 $0.012\%$，
...
随着分布式光伏在配电网中的渗透率不断提高，就地消纳光伏发电、减少对大电网的
依赖，成为能源系统运行中的一个现实问题。微网正是为解决这一问题而发展起来的小型
发用电系统，由分布式电源、储能装置与本地负载构成；非孤岛运行的微网还与外部电网
相连，可以在电价较低或分布式能源有剩余电量的时段为储能充电、在电价较高的时段
放电以获得收益。本题中的微网需要依据小区负载、光伏发电量与储能当前储电量，
每天 0:00 给出当天的计划购电量与储能充放电量，使供给的电力既能满足小区负载、
又尽可能节省购电费用。难点有两方面：一是光伏出力与负载都具有随机性，当天的
实际情况未必与预期一致；二是储能的容量、功率与效率都受硬约束，购电决策必须在
买少了须按高价补购、买多了用不掉也须照付之间权衡。
...
问题一处理最理想的情形：电价与小区负载每天相同、光伏出力也有逐 10 分钟预测
数据，全部输入均为已知；要求在微网所供电能不低于小区负载、且储电量在 0:00 与
24:00 相同的条件下，建立每天 0:00 制定当天计划购电策略的数学模型，并给出具体的
计划购电策略。问题二放松“负载与光伏每天相同”这一条件：电价仍不变，但负载与
光伏逐日逐时段变化；若微网所供电能低于负载，缺口部分需按交易时刻电价的 5 倍
紧急购电，其余时段的购电费用按计划购电量结算。问题三进一步引入预报信息：每天
0:00、6:00、12:00、18:00 可获得未来 24 小时整点的光伏功率预报，因此除在 0:00
制定计划外，还可在其余三个时刻依据新预报调整计划；计划量高于调整量的部分按交易
时刻电价的 50\% 计违约金，调整量高于计划量的部分按 1.5 倍计价，总费用由计划购电费、
紧急购电费与调整购电量的相关费用构成，并要求分析是否需要引入其他时刻的预报。
...
\begin{tikzpicture}[
  ...
  \node[q3b, anchor=north west] (q3) at (0,-7.3) {%
    ...
    \textbf{校正}：预报水平校正，值 8.44 万元\\[1pt]
    \textbf{结果}：总购电费 \textbf{13\,641\,420 元}\\[1pt]
    \hspace{1.6em}紧急购电 53\,696 kWh\\[1pt]
    \hspace{1.6em}高出同口径完美预见下界 $11.55\%$};
  ...
  \node[q4b, anchor=north east] (q4) at (15.85,-7.3) {%
    ...
    \textbf{分位}：与缺口相关，须五维重扫\\[1pt]
    \textbf{结果}：同一分位口径下账单 $\times$1.0559\\[1pt]
...
问题一给出单日 144 个 10 分钟区间的电价、负载与光伏预测功率；题面又规定每天的电价与
小区负载相同，故该问中电价、负载、光伏三者\emph{全部已知}。因此这里不存在任何随机性，
计划购电与实际购电完全重合。

问题的本质是在一组线性约束下最小化购电费用，是一个标准的\emph{确定性线性规划}，
其经济实质是储能套利，即把电量从低电价时段转移到高电价时段。建模时需处理三件事：
充放电各打 9 折带来的往返能量损失、未定义售电价因而多余光伏只能弃光、以及同一区间
同时充放电的退化性（其代数说明见 \S5.1）。
...
可存进电池、少买可由电池放电顶住，两侧代价都被削平，故最优分位只能由数值扫描确定。
...
\emph{读法}问题。本文按字面读法\emph{不设}上界（假设 6），保守读法的对照见
\S\ref{sec:sens}。
...
\noindent（1）计费口径须逐句对齐题意。题面问题 3 规定总费用由“计划购电费用、紧急
购电费用和调整购电量的相关费用”三部分组成，而问题 2 又规定其他时间段的购电费用
“均按计划购电量计算”，故计划购电量按全额付费（take-or-pay 口径），计算公式如下所示：
...
$e_t$ 为紧急购电量；其中违约与超额两个倍率 $0.5p$、$1.5p$ 直接来自题面设定。
...
补购，其静态临界比即 \S2.3 之(2) 的 $0.80$；后三块可调，缺电可用 $1.5p$
上调兜底，纯理论临界比为 $1/3$。\textbf{但两个理论值都被实测
...
\noindent（4）预报误差的两支成分与水平校正。预报误差可分解为“整天整体偏高或偏低”
与“日内形状偏差”两支，前者强自相关，仅取残差分位数抓不到，需用当天已发生时段的
实测与基准之比作水平校正（做法见 §5.3）；\S\ref{sec:q3result} 的消融显示这一项值
8.44 万元。
...
\noindent（1）价格结构。对附件 4 的全年数据作三项统计：逐槽跨日均值剖面与附件 1 基准
剖面的最大绝对差为 $5.151\times10^{-5}$ 元，说明附件 4 就是“典型日分时电价形状 $+$ 逐日
波动”；方差分解给出日内形状占 $84.6\%$、日水平因子占 $9.2\%$、日内残差占 $6.2\%$；
日均价均值 0.7662 元/kWh、$std/mean=13.5\%$；日水平因子 $a_d$ 与日均净负荷（当日负载
减光伏，折合日电量）的相关系数达 $0.982$，即\emph{缺电日的电价系统性偏高}。三者的图示见
图 \ref{fig:q4price}：(a) 各日减去当天均值后的形状与附件 1 基准剖面几乎重合，(b) 日水平
因子的分布，(c) 上述相关性。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_q4_price.png}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_q4_priceheat.png}
  \caption{附件 4 电价的全年热力图：(a) 实际电价，(b) 逐日减去当天均价后的形状}
  \label{fig:q4priceheat}
\end{figure}

\noindent 图 \ref{fig:q4priceheat} 把同一结构放到整年上看：(a) 的横向明暗即日内形状、
纵向明暗即日水平因子；(b) 逐日减去当天均价后，\emph{365 天的竖条几乎完全重合}——
日内各时段的相对高低不随日期变化，逐日波动几乎全部是整体平移，偶见的深色块即那
$6.2\%$ 的日内残差。这既是“日内形状占总方差 $84.6\%$”的直观形式，也是问题四把计划层
价格取为公布剖面的依据。

\noindent（2）由此得到的结论。式 \eqref{eq:q3cost} 的四项收费
$p\hat g$、$0.5p$、$1.5p$、$5p$ 都是同一个 $p$ 的倍数，而日水平因子是\emph{全天
均匀平移}；均匀平移不改变任何边际权衡。因此在\emph{同一分位口径}下，电价的整体水平
只按比例影响账单、不改变决策，能进入决策的只有日内残差（占总方差 $6.2\%$）。
但需注意：日内残差虽小，它与缺口\emph{同时}发生的那一部分会改变边际成本比，
从而移动\emph{最优分位本身}——问题四的交付分位因此与问题三重扫后不同（见 \S5.4）。

\noindent（3）模型改动范围。问题四不改变储能物理、决策变量边界、可交付结构与结算口径，
只改变价格的进入方式：计划层的价格系数取公布的分时电价剖面（\emph{部分信息口径}，即
0:00 不知道当天实际电价），或取当天实际电价（\emph{完全信息口径}，用作价格信息的下界）；
结算与写盘一律使用附件 4 的实际价。问题四-二另把紧急购电的场景系数由标量 $5p$ 改为
逐场景逐槽 $5p_{\omega,t}/K$，使价格与缺口的联合分布进入决策。

\subsection{数据预处理}
\label{sec:preproc}

\noindent（1）附件 3 只给出\emph{整点时刻}的光伏预报，而解算需要 10 分钟粒度的逐槽
...
\noindent（2）负荷没有预报，须自行构造基线。本文按暖机期的日均净负荷把
七天分为低需求日与其余两组，以回看窗口内同组历史日的均值作为负荷基线，其构造
详见 \S\ref{sec:q2scen}；问题三采用同一构造，仅回看窗口略有不同。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.76\textwidth]{figures/fig_q2_netheat.png}
...
周六。分组为何必须由数据推出、不能按日历直觉设定，以及它与不分组做法的量化差距，
均在 \S\ref{sec:q2scen} 给出。
...
为简化模型并保证可解性，本文作出如下假设（全文符号含义见表 \ref{tab:notation}）：
...
表 \ref{tab:q1t1} 与表 \ref{tab:q1t2} 按题目表 1、表 2 的格式给出指定时间段与全天的
结果。
...
故图 \ref{fig:q1day}(b) 进一步给出储电量的时序轨迹与电价背景的对应关系。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.64\textwidth]{figures/fig_q1_day.png}\\[3pt]
  \includegraphics[width=0.66\textwidth]{figures/fig_q1_soc.png}
  \caption{问题一：(a) 全天电价、负载、光伏与计划购电量；(b) 分块充放电功率与
  储电量轨迹}
  \label{fig:q1day}
\end{figure}
...
水位，因此情景集怎么构造是问题二中最关键的一步建模决策。

本文的构造依据来自附件 2 中一个容易被忽略的事实：真正区分各日的并不是星期，而是
\emph{用电水平}（见图 \ref{fig:q2netheat}）。全年口径下，周五与周六的日均净负荷
分别为 22\,481 与 22\,887 kWh/日，其余五天均在 6.8 万 kWh/日 上下，两者相差近三倍；
周日则不然，其日均净负荷为 68\,386 kWh/日，与周一至周四基本持平，呈现典型的工作日
特征。
...
本分组与常规做法的差别不在取样窗口，而在\emph{分组依据}：常规做法按日历（同星期几）
取样，本文按数据推出的用电水平分组，并只喂暖机期以保证无前视。表 \ref{tab:q2scen}
的比较中，低需求日分组相对对照组节省 128.76 万元（$8.51\%$），这一降幅超过本文在
问题二上任何一项算法改动的效果；改按日历周末取样反而比对照组高 697.04 万元，
完全不分组亦高出 126.92 万元，原因是数据中的周日是工作日，把它与周六并入同一
情景集等于混淆了两种用电水平。

回看窗口取 6 天有其必然性：低需求日每周仅两天，取 6 天才能使周五与周六各得
1 个同组历史日（窗口更短的情形见表注）。窗口延长至 14 天以上则引入季节漂移，
总费回升至 1\,434.17 万元，21 天为 1\,471.35 万元。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.66\textwidth]{figures/fig_q2_year.png}
...
四层因果结构，如表 \ref{tab:q3layer} 所示，求解流程如图 \ref{fig:q3flow} 所示。

\begin{figure}[!htbp]
  \centering
  \begin{tikzpicture}[node distance=6.5mm and 6mm]
    \node[databox] (a) {附件 3\\四档整点预报};
    \node[box, right=of a] (b) {低需求日分组\\构造情景集};
    \node[box, right=of b] (c) {两阶段计划层 LP\\$\hat g$ 一阶段承诺\\$c,d,e,S$ 逐情景追索};
    \node[box, below=of c] (d) {6/12/18 点\\水平校正 $+$ 只上调};
    \node[box, left=of d] (e) {逐槽执行\\严格储电量因果策略};
    \node[warnbox, left=of e] (g) {哨兵检验\\$\ge$ 完美预见下界};
    \node[resbox, below=of e] (f) {输出 result3.xlsx};
    \draw[ar] (a) -- (b); \draw[ar] (b) -- (c); \draw[ar] (c) -- (d);
    \draw[ar] (d) -- (e); \draw[ar] (e) -- (g); \draw[ar] (e) -- (f);
  \end{tikzpicture}
  \caption{问题三的求解流程}
...
逐情景的充放电量，后者是效益的主要来源。实测日均充电量为 19\,847 kWh，
往返效率 $d/\eta\div c=\eta^2=0.81$。
...
预报，由附件 3 的整点预报按 \S2.6 之(1) 的\textbf{线性插值}给出（若按分段常数保持，
正午峰值会被削成台阶）。$S_0$ 取当日实测起始
储电量，$S_T=6000$ 是自加假设（假设 7）。购电功率按题面字面读法\textbf{不设上界}
（假设 6）。对冲分位按 \S2.4 之(3)分块设定：
...
储电量 $SOC$，目标是修正后续时窗的目标净需求。其关键机制是\emph{水平校正}
（机理见 \S2.4 之(4)）：用当天已发生时段的实测负荷与基准负荷之比测出偏差，
再按该比值缩放剩余时段的预报。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l l l}
    ...
    结果 $\ge$ 完美预见下界 & $\ge$ 12\,229\,461 & 13\,641\,420 元 $\checkmark$\\
    储能能量守恒 & $d/\eta\div c=\eta^2$ & $5\,372\,355/6\,631\,807=0.8101$ $\checkmark$\\
...
再把交付的 \texttt{result3.xlsx} 读回来重算：113 个无紧急购电日的费用精确对账，
最大残差 0.005238 元；储电量递推残差 $3\times10^{-4}$ kWh，跨日衔接 0.0000 kWh；
全年合计 13\,641\,420.40 元。交付文件可被第三方按同一口径独立核出。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l r l}
    ...
    \hd{指标} & \hd{数值} & \hd{单位}\\
    \midrule[1pt]
    计划购电量 $\sum\hat g_t\Delta t$ & 20\,587\,146 & kWh\\
    最终调整购电量 $\sum g_t\Delta t$ & 21\,404\,587 & kWh\\
    紧急购电量 $\sum e_t\Delta t$     & 53\,696      & kWh\\
    \midrule[1pt]
    计划购电费 $\sum p\hat g\Delta t$ & 12\,553\,753 & 元\\
    违约费 $0.5\sum p(\hat g-g)^{+}\Delta t$ & 0 & 元\\
    超额费 $1.5\sum p(g-\hat g)^{+}\Delta t$ & 780\,043 & 元\\
    紧急购电费 $5\sum p e\Delta t$    & 307\,624     & 元\\
    \midrule[1pt]
    总购电费 $Z_3$ & 13\,641\,420 & 元\\
    \midrule[1pt]
    储能充电 / 放电 & 6\,631\,807 / 5\,372\,355 & kWh\\
...
相对同口径完美预见下界 12\,229\,461 元高出 $11.55\%$，即预报误差的代价约 140 万元。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{c c c c c c c}
    ...
    \hd{日期} & \hd{时间段} & \hd{购电量/kWh} & \hd{时间段} & \hd{购电量/kWh} & \hd{时间段} & \hd{购电量/kWh}\\
    \midrule[1pt]
2025.3.20 & 10:00-10:10 & 0.00 & 12:00-12:10 & 716.24 & 14:00-14:10 & 0.00\\
 & 16:00-16:10 & 658.11 & 18:00-18:10 & 693.48 & 20:00-20:10 & 0.00\\
\multicolumn{6}{l}{\hd{全天购电量 73\,250.48 kWh；全天购电费 45\,581.98 元}}\\
\midrule[1pt]
2025.6.21 & 10:00-10:10 & 0.00 & 12:00-12:10 & 0.00 & 14:00-14:10 & 0.00\\
 & 16:00-16:10 & 166.11 & 18:00-18:10 & 0.00 & 20:00-20:10 & 0.00\\
\multicolumn{6}{l}{\hd{全天购电量 34\,210.18 kWh；全天购电费 19\,374.02 元}}\\
\midrule[1pt]
2025.9.23 & 10:00-10:10 & 0.00 & 12:00-12:10 & 379.97 & 14:00-14:10 & 0.00\\
 & 16:00-16:10 & 580.23 & 18:00-18:10 & 745.89 & 20:00-20:10 & 0.00\\
\multicolumn{6}{l}{\hd{全天购电量 67\,245.13 kWh；全天购电费 42\,568.11 元}}\\
\midrule[1pt]
2025.12.21 & 10:00-10:10 & 0.00 & 12:00-12:10 & 948.57 & 14:00-14:10 & 0.00\\
 & 16:00-16:10 & 845.85 & 18:00-18:10 & 680.36 & 20:00-20:10 & 0.00\\
\multicolumn{6}{l}{\hd{全天购电量 94\,926.79 kWh；全天购电费 61\,559.30 元}}\\
    \bottomrule[1.5pt]
  \end{tabular}
\end{table}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{c c c c c c c}
    ...
    \hd{日期} & \hd{时间段} & \hd{充电量/kWh} & \hd{放电量/kWh} & \hd{时间段} & \hd{充电量/kWh} & \hd{放电量/kWh}\\
    \midrule[1pt]
2025.3.20 & 0:00-4:00 & 4,462.84  &  157.90 & 4:00-8:00 & 962.93  &  5,098.54\\
 & 8:00-12:00 & 6,291.46  &  2,777.38 & 12:00-16:00 & 3,690.04  &  254.72\\
 & 16:00-20:00 & 13.13  &  5,701.96 & 20:00-24:00 & 5,530.01  &  3,055.57\\
 & \multicolumn{2}{l}{\hd{0:00 储电量 6,142.95}} & \multicolumn{3}{l}{\hd{24:00 储电量 6,058.23}}\\
\midrule[1pt]
2025.6.21 & 0:00-4:00 & 341.52  &  527.53 & 4:00-8:00 & 451.95  &  4,412.75\\
 & 8:00-12:00 & 10,095.91  &  0.00 & 12:00-16:00 & 0.00  &  0.00\\
 & 16:00-20:00 & 58.98  &  5,854.88 & 20:00-24:00 & 5,803.88  &  2,614.38\\
 & \multicolumn{2}{l}{\hd{0:00 储电量 6,488.76}} & \multicolumn{3}{l}{\hd{24:00 储电量 6,666.29}}\\
\midrule[1pt]
2025.9.23 & 0:00-4:00 & 4,402.36  &  119.89 & 4:00-8:00 & 806.02  &  6,406.09\\
 & 8:00-12:00 & 4,579.77  &  1,896.62 & 12:00-16:00 & 6,036.79  &  403.62\\
 & 16:00-20:00 & 70.62  &  5,838.26 & 20:00-24:00 & 5,494.14  &  2,891.70\\
 & \multicolumn{2}{l}{\hd{0:00 储电量 6,364.46}} & \multicolumn{3}{l}{\hd{24:00 储电量 6,108.32}}\\
\midrule[1pt]
2025.12.21 & 0:00-4:00 & 4,844.16  &  59.72 & 4:00-8:00 & 1,691.61  &  1,630.47\\
 & 8:00-12:00 & 5,495.52  &  7,648.87 & 12:00-16:00 & 7,914.03  &  2,330.15\\
 & 16:00-20:00 & 25.29  &  5,831.21 & 20:00-24:00 & 5,385.34  &  2,895.93\\
 & \multicolumn{2}{l}{\hd{0:00 储电量 5,814.99}} & \multicolumn{3}{l}{\hd{24:00 储电量 5,972.75}}\\
    \bottomrule[1.5pt]
  \end{tabular}
\end{table}
...
2025.12.21 & 20:30-20:50 & 19.8019\\
 ...
 & \hd{当日合计} & \hd{66.29}\\
  ...
  \vspace{2pt}
  \parbox{0.95\textwidth}{\zihao{-5} 注：购电量单位为 kWh，四个指定日期合计 153.50 kWh。}
\end{table}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_q3_windows.png}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 调整在电量口径上也是单边向上的：四个时窗的下调量\emph{全部为 0}，
        年净调整量 817\,441 kWh 全部来自上调。这是 \S2.4 之(2) 那条推论的直接实证。
  \item 调整量集中在两个可调时窗：6:00--12:00 为 31.52 万 kWh、
        12:00--18:00 为 33.96 万 kWh，两者合计占净调整总量的 $80.1\%$；
        而 0:00--6:00 虽计划量最大（790.82 万 kWh），净调整仅 7.56 万 kWh（$9.2\%$），
        这正是盲窗无调整机会在电量上的体现。
  \item 紧急购电量集中在入夜后：18:00--24:00 为 41\,435 kWh、6:00--12:00 为
        10\,653 kWh、12:00--18:00 为 0，盲窗仅 1\,607 kWh。
        说明在可调时窗内调整通道已把缺口基本补齐。
\end{enumerate}

\noindent（4）对冲分位是平台中心，不是尖点最优。图 \ref{fig:q3tau} 给出两条一维扫描：
$\tau'$ 的两端都翘起来（修复锚点前的对照测量中，$\tau'=0.10$ 时紧急购电
471\,422 kWh，为定稿的 8.8 倍，调整层形同关闭），$\tau_0$ 同样在中间取到最优。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_tau.png}
  \caption{对冲分位的敏感性}
  \label{fig:q3tau}
\end{figure}

曲面在最优附近是一个\emph{宽平台}：在两个分位各自的扫描区间
$\tau_0\in[0.50,0.55]$、$\tau'\in[0.42,0.44]$ 上，总费落在
13\,639\,718--13\,641\,420 元，极差仅 \textbf{1\,702 元（$0.012\%$）}；
其中最低点 $(0.55,0.44)$ 比交付点低 1\,702 元。差异已在平台噪声之内，
故定稿取 $(\tau_0,\tau')=(0.55,0.42)$，本文\emph{不声称}求得尖点最优。

\textbf{分位是计划层的伴生参数，换计划层或动预报锚点都必须重扫}：确定性计划层
时代调出的 $(0.80,0.55)$ 原样搬到追索版会偏贵 185\,974 元。以固定 $\tau_0$ 的一维
扫描为例，$\tau'$ 由 $0.55$ 下调到 $0.42$ 时，计划购电费减少 48.4 万元（少承诺），
代价是超额费 $+10.6$ 万元、紧急购电费 $+18.6$ 万元，三项净省 19.2 万元。方向与
\S2.4 之(3) 一致：计划层自己按情景对冲后，调整层就不该再叠加一层厚缓冲。
本轮修复预报锚点后重扫，最优点只由 $0.42$ 移到 $0.44$（差 1\,702 元），交付 τ 不变。
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.72\textwidth]{figures/fig_q3_ablation.png}
  \caption{问题三的消融实验}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 日内调整通道是价值最大的一块：关闭后总费升至 14\,690\,130 元，
        即该通道值 104.87 万元，其中 6:00 档 54.30 万元、12:00 档
        46.07 万元、18:00 档 4.50 万元；
  \item 调整层的水平校正值 8.44 万元（机理见 \S2.4 之(4)）；
  \item 18:00 档只值 4.50 万元，比确定性计划层时代的 3\,865 元
        高一个量级——追索计划层把前两档能吃掉的误差吃完了，留给 18:00 档的反而更多。
        这与附件 3 的预报质量分析方向一致（见下节）。
\end{enumerate}

\noindent（6）基线口径的归因。低需求日分组带来的 60.35 万元增益与调整通道的
104.87 万元\emph{不可相加}，因为两者在修同一类偏差，是部分替代品。这也是水平校正的
边际估值从确定性计划层时代的 13.30 万元降到追索版的 8.44 万元的原因：计划层自己
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.54\textwidth]{figures/fig_q3_rmse.png}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item 需要引入 6:00 档，把上午的净需求误差降低 $36.2\%$，在成本口径下
        值 54.30 万元；
  \item 需要引入 12:00 档，把午后的误差降低 $50.1\%$，值 46.07 万元；
  \item 18:00 档可以省掉，因为该时窗已入夜、光伏接近 0，而 0:00 档对夜间净需求
        本来就报得不错（305.9 kW），18:00 档没有带来实质新信息。按成本口径其边际
        价值仅 4.50 万元（$0.40\%$），可省，但并非无价值。
\end{enumerate}

\noindent 电量口径给出一个补充观察：盲窗（0:00--6:00）虽无调整机会，其紧急购电仅
1\,607 kWh，反而远低于 18:00--24:00 的 41\,435 kWh（也低于 6:00--12:00 的
10\,653 kWh）。原因是两段采用了不同的对冲水位：盲窗按 $\tau_0\approx0.55$ 正偏对冲、
...
结构、式 \eqref{eq:q3cost} 的四项结算、334 天窗口）全部沿用。
...
据此，问题四的结算式即把问题三式 \eqref{eq:q3cost} 的四项收费的价格统一换成
附件 4 的实际价 $p_{d,t}$，其计算公式如下所示：
\begin{equation}
Z_4=\sum_t p_{d,t}\hat g_t\Delta t
  +0.5\sum_t p_{d,t}\bigl(\hat g_t-g_t\bigr)^{+}\Delta t
  +1.5\sum_t p_{d,t}\bigl(g_t-\hat g_t\bigr)^{+}\Delta t
  +5\sum_t p_{d,t}e_t\Delta t .
\label{eq:q4cost}
\end{equation}

\noindent 式中 $\hat g_t$ 为计划层按公布剖面制定的计划购电量，$g_t$ 为实际执行的调整
购电量，$e_t$ 为紧急购电量。需强调的是：\emph{结算}一律用实际价 $p_{d,t}$，而计划层
在 0:00 只掌握公布的分时电价剖面（部分信息口径），其目标函数与问题三同形；因此
问题四-三的模型即式 \eqref{eq:q3cost}--\eqref{eq:q3plan} 再加这一层价格分离。

\noindent（2）问题四-二：场景集需联合价格与净负荷。问题三的场景集只依赖
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[node distance=6.5mm and 6mm]
    ...
    \node[box, right=of b, yshift=6mm] (c) {计划层 LP\\价格预期（部分信息口径）\\分位经五维重扫};
    \node[box, right=of c] (d) {6/12/18 点\\水平校正 $+$ 只上调};
    \node[box, below=of d] (e) {逐槽执行\\严格储电量因果策略};
    \node[box, left=of e] (f) {结算层\\按附件 4 实际价结算};
    \node[resbox, left=of f] (g) {输出 result4-3.xlsx\\result4-2.xlsx\\（$G$ 按字面读法放开）};
    \draw[ar] (a) -- (c);
    \draw[ar] (b) -- (c);
...
变化的只有按实际价结算的账单。把问题四-三钉在与问题三\emph{共享}分位上重跑，与交付的
\texttt{result3.xlsx} 逐元素比对：计划购电量、调整购电量、充放电量与紧急购电量
\textbf{相异 $0/48\,096$ 槽}，账单由 13\,641\,420 元升至 \textbf{14\,404\,088 元}
（$\times$1.055908）——这是两份独立交付物的交叉验证，而不是推理。

\noindent 需与其区分的一点：问题四-三的\emph{交付}分位取本问自己的最优（见 \S\ref{sec:q4neg}），
与问题三不同，故两份\emph{交付文件}必然逐格相异（实测 65\,333 格）。\textbf{差异来自
分位，不是来自价格}；\S\ref{sec:sens} 的价格缩放实验进一步确认同一结论——结算价格
整体缩放 $\pm30\%$（13 档）时策略逐元素不变、账单精确线性，最大偏差
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_q4_shift.png}
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l r r}
    ...
    \midrule[1pt]
    总费用 / 元 & \hd{14\,257\,306} & \hd{14\,573\,057}\\
    　计划购电费 & 13\,311\,305 & 13\,953\,352\\
    　违约费 & 0 & ——\\
    　超额费 & 683\,620 & ——\\
    　紧急购电费 & 262\,382 & 619\,705\\
    　紧急购电量 / kWh & 44\,654 & 105\,864\\
    计划购电量 $\sum\hat g$ / kWh & 20\,552\,760 & 21\,734\,671\\
    实际购电量 $\sum g$ / kWh & 21\,237\,452 & 21\,734\,671\\
    储能充电 / 放电 / kWh & 6\,633\,097 / 5\,373\,100 & 6\,396\,664 / 5\,183\,584\\
    真实储电量区间 / kWh & $[1\,200,\ 10\,800]$ & $[1\,200,\ 10\,800]$\\
    年末储电量 / kWh & 6\,107 & 6\,219\\
    完美预见下界（同口径）/ 元 & 12\,782\,592 & 12\,782\,592\\
    \bottomrule[1.5pt]
  \end{tabular}
\end{table}

问题四-三的\emph{交付}值相对问题三为 $+615\,886$ 元（14\,257\,306 对
13\,641\,420）；若两侧统一到共享分位（即上文的受控消融），则为 $+762\,668$ 元
（$\times1.055908$）。增量的构成见图 \ref{fig:q4bill}：计划购电费增加 75.76 万元、
超额费减少 9.64 万元、紧急购电费减少 4.52 万元，相抵后净增 61.59 万元。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{figures/fig_q4_bill.png}
  \caption{问题四与问题三的账单对比}
  \label{fig:q4bill}
\end{figure}

策略变化为 0（受控消融口径）。问题四-二则不同：其场景系数 $5p_{\omega,t}/K$ 进入了
目标函数，策略\emph{确实改变}，交付总费 \textbf{14\,573\,057 元}，相对问题二的
13\,846\,553 元高 726\,504 元（$\times$1.052468）；若不引入该系数，其计划与问题二的
交付计划逐槽等价。

\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.79\textwidth]{figures/fig_q4_dailybill.png}
  \caption{问题四的逐日账单与逐日倍率}
  \label{fig:q4dailybill}
\end{figure}

\noindent 图 \ref{fig:q4dailybill} 把账单变化放到逐日尺度：(a) 两份交付文件的逐日账单；
(b) 其逐日比值——\textbf{334 天全部变动}，倍率在 $0.71$ 至 $1.25$ 之间，其中 136 天
反而更便宜，而\emph{合计之比}为 $\times$1.0451。注意逐日倍率的平均值（1.0030）与合计
之比并不等价，两者不可混用。这说明换价连同分位重扫在逐日尺度上是有增有减的，
只是总账为增。

\noindent（3）价格信息的价值。完全信息口径指 0:00 即可知当天实际电价，
此时计划层不必再用公布的分时电价作预期。两种口径的总费差额即价格信息的价值，
实测为 \textbf{126\,247 元}（占 $0.885\%$），说明价格的水平信息对决策几乎无用，
这与 \S2.5 的方差分解互为印证。问题四-三的主交付档位见表 \ref{tab:q4matrix}。
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l r r r}
    ...
    \midrule[1pt]
    部分信息口径，无上界读法，本问五维最优分位 & 14\,257\,306 & 44\,654 & $11.54\%$\\
    \bottomrule[1.5pt]
  \end{tabular}

  \vspace{2pt}
  \parbox{0.95\textwidth}{\zihao{-5} 注：本档位的分位是本问自己的五维最优，与问题三
  不同；与问题三的受控对照（共享分位、决策完全不变）见上文。}
\end{table}

\subsubsection{参数重扫与负结果}
\label{sec:q4neg}

\noindent（1）对冲分位必须随价格结构重扫。问题三的分位是在\emph{常数价格}下扫出来的；
问题四下价格与缺口正相关（图 \ref{fig:q4price}(c) 的 $corr=0.982$），这会改变有效
成本比 $c_u=4E[p\,|\,\text{缺口}]$ 与 $c_o=E[p\,|\,\text{过剩}]$，最优分位随之移动。
本文把四个发布时刻的分位与调整层分位\emph{一起}放开重扫（五维），问题四-三的交付分位
为 $(0.58,0.36,0.40,0.70)$、$\tau'=0.28$，与问题三的 $(0.55,0.42)$ 不同。
\textbf{这是一条正面结论}：分位是计划层与价格结构的伴生参数，换一套价格机制就要重扫。

\noindent 一处自洽性检查：若把四个时刻的分位绑成一条（即退回四象限扫描的粗网格），
同一配置下的最优值为 14\,398\,996 元，比交付的 14\,257\,306 元贵 141\,690 元——
差的正是“把五维绑成一维”的代价；两条搜索路径互相印证，也说明该结论不是扫描噪声。

\noindent（2）电价加权分位：一个负结果。报童临界比是 $c_u/(c_u+c_o)$，
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{l l l l}
    ...
    \midrule
    对冲分位 $\tau'$ & $0.42\sim0.44$ & 总费极差 $0.012\%$ & 极稳健（平台区）\\
    对冲分位 $\tau_0$ & $0.50\sim0.55$ & 同上（平台为二维） & 极稳健（平台区）\\
    覆盖率 $\gamma$ & $0\sim1$ & 总费单调，差 $13.5\%$ & 不稳健，故锁定 $\gamma=1$\\
    结算电价 & $\pm30\%$（13 档） & 策略逐元素不变，账单精确线性 & 极稳健\\
    低需求日分组 & 开 $\to$ 关 & 问题三 $+60.35$ 万 / 问题二 $+128.76$ 万 & 显著更优，保留\\
    购电上界 $G_{\max}$ & 加/去上界（读法） & 确定性 $+12.7$ 万 / 追索 $-27.8$ 万 & 读法选择，须分层\\
    光伏预报档位 & 三档 $\to$ 只用 6:00 档 & $+50.57$ 万 & 6:00/12:00 两档必需\\
    水平校正 & 开 $\to$ 关 & $+8.44$ 万 & 必需\\
    \bottomrule[1.5pt]
  \end{tabular}
\end{table}
...
\begin{figure}[!htbp]
  \centering
  \includegraphics[width=0.59\textwidth]{figures/fig_q4_pscale.png}
  \caption{结算价格 $\pm30\%$ 的敏感性}
  \label{fig:q4pscale}
\end{figure}

\noindent（2）购电上界：方向取决于计划层，不能脱离计划层单独回答（读法问题见假设 6）。
在确定性计划层下放开上界\emph{更贵}
（14\,289\,296 $\to$ 14\,415\,949 元，$+12.7$ 万），原因是日前线性规划把电池过度投入
价格套利、挤掉了可靠性；而在\emph{追索}计划层下放开上界\emph{更便宜}
（13\,919\,114 $\to$ 13\,641\,420 元，$-27.8$ 万），因为追索写法把“承诺量”与
“情景内怎么用电池”分开，套利不再挤占可靠性。本文交付按字面读法放开，保守读法的
可交付值见 \S\ref{sec:q3lim}。
...
交付文件经\emph{不引用任何求解器}的独立脚本复核：113 个无紧急购电日的费用
精确对账，最大残差 0.005238 元。
...
静态报童临界比给出的 $0.80$ 与 $1/3$ 均被实测推翻，分位只能扫描、且必须随计划层重扫。

\noindent（3）结论的稳健性有量化边界。$\tau$ 的平台区极差仅 $0.012\%$、
电价 $\pm30\%$ 下策略逐元素不变、账单精确线性，这些不是“模型稳健”的套话，
而是可复现的定量区间（表 \ref{tab:sens}）。

\noindent（4）对文献结论给出了正面回应。已有研究
\cite{parra2016} 报告动态电价使\emph{收益}提高 $28\%$、\emph{成本}提高 $94\%$。
本文的结论与之不矛盾而落点不同：微网是\emph{纯购电方}（题面只给购电价、输出表只有
购电量），故“收益 $+28\%$”在本模型中无对应量；在纯购电费用口径下波动电价的效应仅
$+5.05\%$、且决策完全不变，远小于文献的 $+94\%$（后者含电池退化等成本项）；
而本文不计退化成本（假设 8），故成本增幅会高于 $5.05\%$。文献 \cite{parra2016} 的
$\pm30\%$ / $5\%$ 步长敏感性本文已照搬（\S\ref{sec:sens}，13 档）。
...
\noindent（2）情景集的构造规则取自同一份数据，尚缺样本外检验。它的构造只用计费
窗口之前的数据、不存在前视，但与评估窗口毕竟来自同一份附件，故本文只主张该构造在
报告窗口上显著更优，不主张其在任意数据集上都成立；分布无关的鲁棒性需在独立数据集上
作样本外检验，本文未做。

\noindent（3）不计电池退化成本。由假设 8，模型倾向于更充分地使用电池
（问题三全年储能充电 6\,631\,807 kWh，约合 690 次等效满循环）。若引入退化成本
...
声明并在 \S\ref{sec:sens} 做敏感性。购电上界的性质与两种读法的分界见假设 6 与
\S\ref{sec:q3lim}；若换一个电网，实际联络线容量会直接决定交付表中贴界槽的占比，
必须重新标定。
...
\noindent（1）推广至一般“价差套利 $+$ 缺货惩罚”的储能调度场景。本文框架只需替换
价格机制与缺货惩罚倍数两组输入：例如\emph{工业园区储能电站}的价格机制为尖峰电价
$+$ 需量电费，把式 \eqref{eq:q3cost} 的 $5p$ 换成需量电费对应的边际惩罚即可照用。

\noindent（2）推广至任何“预先承诺 $+$ 事后调整”的两阶段定价合同。本题的计费口径
本质是一份 take-or-pay 合同：计划量全额付费、偏差按 $0.5p/1.5p$ 非对称结算；
凡具有该结构的合同（如电力中长期交易），都可用本文的分块分位对冲规则定出报计划量。

% ---- 并入：结果的口径与适用范围 ----
\subsection{结果的口径与适用范围}
\label{sec:scope}
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  \item $\tau$ 是平台中心而非尖点最优（$\tau_0\in[0.50,0.55]\times\tau'\in[0.42,0.44]$
        极差 1\,702 元，$0.012\%$），本文不声称求得最优分位；且\textbf{分位随计划层
  ...
  \item 18:00 档只值 45\,016 元（$0.33\%$），与附件 3 的预报质量矩阵方向一致。
...
\begin{enumerate}[itemsep=1pt, topsep=2pt]
  ...
  \item 购电上界同为\emph{读法选择}（见问题三第 2 条）。方案阶段曾预留一条待验证的
        猜想：波动电价让套利激励更强时，“上界反而更优”这条结论有可能翻转；在前一版
        基准下实测没有翻转，放开上界仍更贵；该方向需在问题三新基准下重测确认。
  \item 哨兵必须与读法配套：问题四的哨兵在\emph{无上界}读法下为 12\,782\,592 元（与问题三
        同为该读法），在 $G\le5000$ kW 读法下为 13\,014\,628 元；两者都是真下界，
        但“高出哨兵 $X\%$”只能在同一读法内比较，跨读法并列时必须注明。
\end{enumerate}
...
% ===================================================================== 参考文献
% 参考文献单列一页；标题由 cumcmthesis.cls 的 thebibliography 环境自带
% （\section*{\refname}），故此处不再写 \section{参考文献}，否则会出现两个标题。
\clearpage
...
\noindent 下列四条是正文各处引用的口径约定，集中于此以便对照；数据本身的加工（整点预报插值、负荷基线构造）留在 \S\ref{sec:preproc}。
...
\noindent（3）因果锚点：不取前视。与插值配套的还有一处时间对齐细节——附件 1/2 的槽按\emph{右端点}标注，因此
$S$:00 发布时刻可用的最后一个\emph{已观测}槽是 $(S{-}1)$:50--$S$:00。调整层的锚点取
这一槽，而不是 $S$:00--$S$:10 那一槽——后者属于发布\emph{之后}的实测，若取它等于把
未来 10 分钟的信息当成已知（前视），会系统性低估成本。本文的因果锚点按此设定。


\noindent（4）统计窗口。问题 2、3、4 统一取 2025 年 2 月 1 日至 12 月 31 日
共 334 天，1 月作为暖机期。
